...
int init_processes()
{
    ...
    for (uint8_t i = 0; i < PRIORITY_LEVELS; i++)
    {
        ...
        {
            return -1; // Error al crear la cola de procesos
        }
    }
...
void set_next_process(void * current_stack)
{
    ...
    {
        return; // No hay proceso actual o no está en ejecución
    }
    ...
    if (current_process->pid == 0) // No encolar el proceso idle
    {
        return;
    }
...
void * get_next_process()
{
    ...
    if (next_process == NULL) {
        next_process = (PCB *)get_idle_process();
    }
