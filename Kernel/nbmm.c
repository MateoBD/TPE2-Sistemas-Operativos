...
void *my_malloc(uint64_t size) {
  ...
  if (buddy_man.free_blocks[order] == NULL) {
    ...
    for (uint8_t i = order + 1; i <= buddy_man.max_order && order_approx == 0;
         i++) {
...
void my_free(void *address) {
  ...
  block_t *buddy_block =
      (block_t *)((uint64_t)MEM_START +
                  (((uint64_t)block_pos) ^ ((1L << block->order))));
  if (buddy_block == NULL) {
    return;
  }
  while (block && buddy_block && block->order < buddy_man.max_order &&
    ...
    buddy_block =
        (block_t *)(block_t *)((uint64_t)MEM_START + (((uint64_t)block_pos) ^
                                                      ((1L << block->order))));
  }
